         ...
         # iF COLLISION IS OCCURED, IT WILL CHANGFE THE BALL POSIOTION AND UPDATES THE POINTS
            if isCollision(player, goals[count]):
                goals[count].setposition(
                    randint(-290, 310), randint(-290, 310))
...
# SHOW THE MESSAGE FOR NOT HAVING ENOUGH MONEY AND GO TO EARNING MONEY GAME
    if amount < 10:
        ...
        zero.end_fill()
        zero.goto(-90, -60)
        zero.write("YOU HAVE LOW MONEY\nTO PLAY THE GAME\nPRESS e TO EARN MONEY", font=(
            "ARIAL", 18, "bold"))
        listen()
        onkey(earnmoney, "e")


# IT WILL CREATE THE GROUND FOR RACING
...
def ground(col1):
    ...
    for j in range(10):
        finish.setpos(finish_line+square_size,
                      ((-10-square_size)-(j*square_size*2)))
        finish.stamp()
    finish.hideturtle()
...
def userin():
    ...
    global betturt
    betmoney = numinput(
        "Betting Money", "Enter the amount for betting", minval=10, maxval=amount)
    betturtle = textinput(
        "Bet Turtle Name", "Enter the Name of the Turtle You want to put Bet")

    # CONVERTING THE INPUT TO UPPERCASE
    betturt = betturtle.upper()

    # sHOWING MESSAGE IF THE NAME OF THE BETTING PLAYER DOESN'T MATCH ANY PLAYER NAME
    if((betturt != p1) and (betturt != p2) and (betturt != p3) and (betturt != p4) and (betturt != p5)):
        messagebox.showinfo(
            "Wrong Turtle Name", "The Turtle Name you Entered\nDoesn't match with any\nPlayer name")
        userin()
...
def maingame(col):
    ...
    if(plnamesdecision == "YES" or plnamesdecision == "Y"):
        for i, j in pldict.items():
            if(no <= plno):
                pldict[i] = textinput(
                    "PLAYER NAME", "ENTER PLAYER "+str(no)+" NAME : ")
                no += 1
    else:
        pass
...
# RaceLight
    ...
    # DETERMINING THE WINNER PLAYER POSITION
